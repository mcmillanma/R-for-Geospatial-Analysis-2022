...
library(prism)
options(prism.path = ".")
prism::get_prism_monthlys(type = "tmean", years = 2015:2015, mon = 1:12, keepZip = F) #temperature mean can switch; 4kmx4km whole USA
...
avalue<-extract(pcp,a) #extraxt values to table or extract milti values to points tool in GIS
plot(avalue[1,])
